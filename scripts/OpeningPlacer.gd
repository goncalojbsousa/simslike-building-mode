...
func _get_wall_centerline(from_tile: Vector2i, to_tile: Vector2i) -> Dictionary:
	var from_w := GridManager.tile_to_world(from_tile)
	var to_w := GridManager.tile_to_world(to_tile)
...
func _find_wall_under_mouse() -> Dictionary:
	...
	for offset in candidates:
		...
		if not _has_wall_on_floor(tile, neighbor, FloorManager.current_floor):
			continue
		var key := _make_wall_key(tile, neighbor, FloorManager.current_floor)
		...
		if perp_dist < best_dist and perp_dist < GridManager.TILE_SIZE * 0.8:
			best_dist = perp_dist
			best = {
				"key":       key,
				"from_tile": tile,
				...
				"start_world": start_w,
				"end_world":   end_w,
				"floor_index": FloorManager.current_floor,
			}
...
func _update_preview() -> void:
	...
	pos.y = GridManager.get_wall_y_base(hit["floor_index"])
